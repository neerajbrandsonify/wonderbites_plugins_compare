...
if(!class_exists('THWEPO_Admin_Settings_Import_Export')):

class THWEPO_Admin_Settings_Import_Export extends THWEPO_Admin_Settings_General{
	...
	public function render_settings($settings){
	    ...
	    <form id="import_export_settings_form" method="post" action="" class="clear">
            <table class="form-table thwepo-settings-table">
                <tbody>
                	<tr>
						<td colspan="3" class="section-title"><?php _e('Backup and Import Settings', 'woocommerce-extra-product-options-pro'); ?></td>
					</tr>
					<tr>
						<td class="label">
							<span><?php _e('Plugin Settings Data', 'woocommerce-extra-product-options-pro'); ?></span>
							<span class="description"><?php _e('You can transfer the saved settings data between different installs by copying the text inside the text box. To import data from another install, replace the data in the text box with the one from another install and click "Save Settings".', 'woocommerce-extra-product-options-pro'); ?></span>
						</td>
						<td class="tip"></td>
						<td class="field">
                ...
                <tfoot>
                	<tr>
						<td colspan="3" class="actions">
							<input type="submit" name="save_plugin_settings" class="button-primary" value="<?php _e('Save Settings', 'woocommerce-extra-product-options-pro'); ?>">
							<!--<input type="submit" name="import_settings" class="button" value="Import Settings(.txt)" onclick="thwepoImportSettings()">
	...
	public function get_posted_settings_data(){
		$settings = THWEPO_Utils::get_posted_value($_POST, 'i_settings_data');
		$settings = $settings ? unserialize(base64_decode($settings)) : '';
		return $settings;
	}

	public function get_imported_settings_data(){
		$file = THWEPO_PATH."wepo-pro-settings-import.txt";
	...
	public function export_settings_data($settings){
		$file = THWEPO_PATH."thwepo-pro-settings.txt";
		...
		if(THWEPO_Utils_Section::is_valid_section($section)){
			...
			if($url){
				...
				$notice = sprintf(
				    ...
				    __( 'Section exported successfully. The exported file is available in <a href="%1$s">Media Library</a> or You can download it <a download href="%2$s">here</a>.', 'woocommerce-extra-product-options-pro'),
				...
				$notice = __('An error occurred while exporting data. Please try again!', 'woocommerce-extra-product-options-pro');
			    add_action('admin_notices', function() use ($notice) {
	...
	public function export_fields($section_name){
		...
		if(THWEPO_Utils_Section::is_valid_section($section)){
			...
			if(!in_array(1, $f_selected)){
				$notice = __('Field(s) not exported. Choose at least one field to export!', 'woocommerce-extra-product-options-pro');
			    add_action('admin_notices', function() use ($notice) {
			        echo '<div class="notice notice-error"><p>'. $notice. '</p></div>';
			    });
			    return;
			...
			if(!empty($fields)){
				...
				if($url){
					...
					$notice = sprintf(
						/* translators: 1: Media library URL 2: Exported file URL */
					    __( 'Field(s) exported successfully. The exported file is available in <a href="%1$s">Media Library</a> or You can download it <a download href="%2$s">here</a>.', 'woocommerce-extra-product-options-pro'),
					    admin_url('upload.php'), esc_url($url)
					);
				    add_action('admin_notices', function() use ($notice){
				        echo '<div class="notice notice-success"><p>'. $notice. '</p></div>';
	...
	public function output_export_form(){
		?>
        <div id="thwepo_export_form_pp" class="thpladmin-modal-mask">
					...
					<div class="modal-content">
						<div class="modal-body">
							<div class="form-wizard wizard">
								<form method="post" id="thwepo_import_form" action="" enctype="multipart/form-data">
									...
									<footer>
										...
										<div class="btn-toolbar">
											...
											<?php wp_nonce_field( 'import_section_fields', 'thwepo_form_security' ); ?>
											<input type="hidden" name="thwepo_form_action" value="import_section_fields">
										</div>
									</footer>
								</form>
							</div>
	...
	private function export_data_to_file($data){
		...
		THWEPO_Utils::write_log($upload['error']);
		return false;
	}

	private function file_name_for_export(){
		$date_time = current_datetime();
		$file_name = 'thwepo-export-'.$date_time->format('Y-m-d-H-i-s').'.txt';
		return $file_name;
	}
	...
	public function import_section_fields($section_name){
		if(empty($_FILES['file_to_import']['tmp_name']) || !is_uploaded_file($_FILES['file_to_import']['tmp_name'])){
			$notice = __('An error occurred while importing file. Please try again!', 'woocommerce-extra-product-options-pro');
			add_action('admin_notices', function() use ($notice) {
		        echo '<div class="notice notice-error"><p>'. $notice. '</p></div>';
		    });
		    return;
		...
		if(!wp_match_mime_types('text/plain', $wp_filetype['type'])) {
			$notice = __('Invalid file type. The importer supports TXT file format.', 'woocommerce-extra-product-options-pro');
			add_action('admin_notices', function() use ($notice) {
		        echo '<div class="notice notice-error"><p>'. $notice. '</p></div>';
		    });
		    return;
		...
		if(empty($file_content)){
			$notice = __('An error occurred while importing file. Invalid file!', 'woocommerce-extra-product-options-pro');
			add_action('admin_notices', function() use ($notice) {
		        echo '<div class="notice notice-error"><p>'. $notice. '</p></div>';
		    });
		    return;
	...
	private function import_section($data){
		...
		if(($object_name == "WEPO_Product_Page_Section") && (THWEPO_Utils_Section::is_valid_section($data))){

			$result = $this->update_section($data);

			if($result){
				$notice = __('Section imported successfully.', 'woocommerce-extra-product-options-pro');
				add_action('admin_notices', function() use ($notice) {
			        echo '<div class="notice notice-success"><p>'. $notice. '</p></div>';
			    });
			    return;
	...
	private function import_fields($data, $section_name){
		...
		if(THWEPO_Utils_Section::is_valid_section($section)){
			...
			foreach ($data as $key => $field){
				if(THWEPO_Utils_Field::is_valid_field($field)){
					if(array_key_exists($key, $fields)){
						...
						if(THWEPO_Utils_Field::is_valid_field($existing_field)){
			...
			if($result){
				$notice = __('Field(s) imported successfully.', 'woocommerce-extra-product-options-pro');
				add_action('admin_notices', function() use ($notice) {
			        echo '<div class="notice notice-success"><p>'. $notice. '</p></div>';
			    });
			    return;
