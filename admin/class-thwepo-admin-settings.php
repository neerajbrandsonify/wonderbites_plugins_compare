...
abstract class THWEPO_Admin_Settings {
	...
	public function __construct($page, $section = '') {
		...
		$this->tabs = array(
			'general_settings'  => __('Product Options', 'woocommerce-extra-product-options-pro'), 
			'advanced_settings' => __('Advanced Settings', 'woocommerce-extra-product-options-pro'), 
			'license_settings'  => __('Plugin License', 'woocommerce-extra-product-options-pro')
		);
	}
	...
	private function prepare_form_field_props($field, $atts = array()){
		...
		$fvalue = isset($field['value']) ? esc_attr($field['value']) : '';
	...
	private function render_form_field_element_radio($field, $atts = array()){
		...
		if($field && is_array($field)){
			...
			foreach($field['options'] as $value => $label){
				...
				$field_html .= '<div class="thpladmin-radio-item">';
				$field_html .= '<input type="radio" name="i_'. $fname .'" id="'. $fid .'" value="'. trim($value) .'" '.$checked.'>';
				$field_html .= '<label for="'. $fid .'">' . esc_html($label) . '</label>';
				$field_html .= '</div>';
			}
		}
		return $field_html;
