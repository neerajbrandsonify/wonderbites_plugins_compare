...
class THWEPO_Utils_Section {
	...
	public static function has_non_hidden_fields($section){
	...
	public static function prepare_section_from_posted_data($posted, $form = 'new'){
		...
		foreach( self::$SECTION_PROPS as $pname => $property ){
			...
			if($pname === 'name'){
				$pvalue = isset($posted[$iname]) ? sanitize_key($posted[$iname]) : $property['value'];
			}else if($pname === 'title' || $pname === 'subtitle'){
		...
		if($sections && is_array($sections) && !empty($sections)){
			foreach($sections as $section_name => $section){
				...
				if($section){
					...
					$section_position = $section->get_property('position');

					if($product_type !== 'variable' && $section_position === 'woo_single_variation_5' && apply_filters('thwepo_remove_variable_specific_section', true)){
	...
	public static function prepare_section_html($section, $product_type = false){
		...
		if(!empty($field_html)){
			...
			$section_name = $section->get_property('name');
			$cssclass .= $section_name ? ' thwepo_' . $section_name : ' thwepo_default';
			$cssclass .= $display_styles ? ' thwepo_' . $display_styles : ' thwepo_default_style';

			if($display_styles && 'default_style' !== $display_styles) {
				$is_show_title = $section->get_property('show_title');
				$title_html = self::prepare_main_title_html($section);

				$subtitle_html = '';
				if($section->get_property('subtitle')){
					$subtitle_html = self::prepare_subtitle_html($section);
				}

				$section_html .= '<div class="extra-options '. trim($cssclass) .'" '.$conditions_data.'>';
				if('accordion_style' === $display_styles){
					$section_html .= '<div class="thwepo-section-title section-title" data-section="'. $section_name .'">';
					$section_html .= $title_html.'</div>';
				}

				if($is_div_model === 'yes'){
					$section_html .= '<div class="thwepo-field-wrapper '. $section_name .'">';

					if(($is_show_title && !empty($title_html)) || !empty($subtitle_html)){
						$section_html .= '<div class="thwepo-section-title-wrap">';

						if($is_show_title){
							$section_html .= $title_html;
						}

						$section_html .= $subtitle_html;
						$section_html .= '</div>';
					}

					$section_html .= $field_html . '</div>';
				}else{
					$section_html .= '<table class="thwepo-field-wrapper '. $section_name .'" cellspacing="0"><tbody>';

					if(($is_show_title && !empty($title_html)) || !empty($subtitle_html)){
						$section_html .= '<tr><td class="thwepo-section-title-wrap" colspan="2">';
				...
				if($is_div_model === 'yes'){
					$section_html .= '<div class="extra-options '. trim($cssclass) .'" '.$conditions_data.'>';
					$section_html .= $section->get_property('show_title') ? self::prepare_title_html($section, $is_div_model) : '';
					$section_html .= $field_html;
					$section_html .= '</div>';
				}else{
	...
	public static function prepare_title_html($section, $is_div_model=false){
		$title_html = '';
		...
		$section_name = $section->get_property('name');
		if($is_div_model === 'yes'){
			return '<div class="section-title" data-section="'. $section_name .'">'.$html.'</div>';
		}
		
		if(!empty($html)){
			$html = '<tr><td colspan="2" class="section-title" data-section="'. $section_name .'">'.$html.'</td></tr>';
		}else{
	...
	public static function prepare_main_title_html($section){
		$title_html = '';
		if($section->get_property('title')){
			$title_class = THWEPO_Utils::convert_cssclass_string($section->get_property('title_class'));
			$title_type  = $section->get_property('title_type') ? $section->get_property('title_type') : 'h3';
	...
	public static function prepare_subtitle_html($section){
		$subtitle_html = '';
		if($section->get_property('subtitle')){
			...
			$subtitle_html .= THWEPO_i18n::__t(wp_kses_post($section->get_property('subtitle')));
	...
	public static function get_ajax_conditions_data($section){
		$data_str = false;
